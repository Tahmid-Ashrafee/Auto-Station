...
function updateHeader() {
    var carDetailsLink = document.getElementById("carDetailsLink");
    ...
    if (isLoggedIn()) {
        ...
        if (carDetailsLink) {
            carDetailsLink.style.display = "inline";
        }
        if (logoutLink) {
        ...
        if (carDetailsLink) {
            carDetailsLink.style.display = "none";
        }
        if (logoutLink) {
...
function handleCarForm(event) {
    ...
    // Get car status
    var availableRadio = document.getElementById("carStatusAvailable");
    var unavailableRadio = document.getElementById("carStatusUnavailable");

    if (availableRadio.checked) {
    carStatus = availableRadio.value;
    } else if (unavailableRadio.checked) {
    carStatus = unavailableRadio.value;
    }

    // Get car colors
    var carColorCheckboxes = document.getElementsByName("carColor");
    var selectedColors = [];

    var carColorCheckboxes = document.getElementsByName("carColor");
    carColorCheckboxes.forEach(function (checkbox) {
        if (checkbox.checked) {
...
function handleCarEntry(event) {
    ...
    var carColorCheckboxes = document.getElementsByName("carColors");
    carColorCheckboxes.forEach(function (checkbox) {
        if (checkbox.checked) {
            selectedColors.push(checkbox.value);
        }
    ...
    for (var i = 0; i < carColorCheckboxes.length; i++) {
        carColorCheckboxes[i].checked = false;
    }
...
function displayCarEntries(pageNumber = 1, entriesPerPage = 2) {
    ...
    // var searchColors = [];
    // var colorCheckboxes = document.getElementsByName("searchColors");
    // colorCheckboxes.forEach(function (checkbox) {
    //     if (checkbox.checked) {
    //         searchColors.push(checkbox.value.toLowerCase());
    //     }
    // });
    var searchColors = Array.from(document.querySelectorAll('input[name="searchColors"]:checked')).map(function (checkbox) {
        return checkbox.value;
    });
    
    var filteredCarEntries = carEntries.filter(function (carEntry) {
        var isMatchingColors =
        searchColors.length === 0 || searchColors.every(function (colors) {
            return carEntry.colors && carEntry.colors.includes(colors);
            });
...
//   });
  ...
  document.getElementById("loginLink").addEventListener("submit", function (event) {
    event.preventDefault();
  ...
  document.getElementById("registerLink").addEventListener("submit", function (event) {
    event.preventDefault();
    var username = document.getElementById("registerUsername").value;
    localStorage.setItem("loggedInUser", username);
    checkLoggedInUser();
